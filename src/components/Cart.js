...
const Cart = () => {
  ...
  const handleQuantityChange = async (id, newQuantity) => {
    ...
    if (item && newQuantity > 0) {
      ...
      try {
        const currentMotorbike = await axios.get(`http://localhost:3001/Motorbikes/${id}`);
        const newStock = currentMotorbike.data.stock + quantityDiff;
  ...
  const handleRemove = async (id) => {
    ...
    if (item) {
      try {
        ...
        const currentMotorbike = await axios.get(`http://localhost:3001/Motorbikes/${id}`);
        const newStock = currentMotorbike.data.stock + item.quantity;
        await updateMotorbikeStock(id, newStock);
        removeFromCart(id);
      } catch (error) {
